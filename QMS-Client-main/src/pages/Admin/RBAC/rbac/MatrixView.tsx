...
export const MatrixView: React.FC<MatrixViewProps> = ({
    ...
    return (
        <div className="space-y-4 animate-fade-in">
            {/* Toolbar */}
            <div className="bg-asvo-card rounded-xl shadow-sm border border-asvo-border p-4">
                <div className="flex flex-wrap items-center gap-4">
                    <div className="relative flex-1 min-w-[200px] max-w-md">
                        ...
                        {abilitySearch && (
                            <button
                                ...
                                className="absolute right-2 top-2.5 text-asvo-text-dim hover:text-asvo-text-mid"
                            >
                                <X size={16}/>
                            </button>
                        )}
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="px-3 py-2 text-xs font-medium text-asvo-text-mid hover:bg-asvo-surface-2 rounded-lg transition"
                        >
                            Развернуть всё
                        </button>
                        <button
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            ...
                            className="rounded border-asvo-border text-indigo-600 focus:ring-indigo-500"
                        />
                        <span className="text-sm text-asvo-text-mid">Компактный режим</span>
                    </label>

                    <div className="ml-auto flex items-center gap-4 text-sm text-asvo-text-mid">
                        ...
                        <thead>
                            <tr>
                                <th className={`
                                    ...
                                    text-left bg-asvo-surface border-b border-r
                                    sticky left-0 z-20
                                    shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]
                                `}>
                                    <div className="text-xs text-asvo-text-dim font-normal uppercase mb-1">Роль</div>
                                    <div className="font-bold text-asvo-text">Название роли</div>
                                </th>
                                ...
                                {groupedAbilities.map(group => (
                                    <th
                                        ...
                                        className={`
                                            ${compactMode ? 'p-1' : 'p-2'}
                                            text-center bg-asvo-surface-2 border-b border-r
                                            ${!group.isExpanded ? 'cursor-pointer hover:bg-asvo-surface-3' : ''}
                                        `}
                                    ...
                                    >
                                        <div className="flex items-center justify-center gap-1">
                                            <button
                                                ...
                                                className="p-1 hover:bg-asvo-surface-3 rounded transition"
                                            >
                                                {group.isExpanded
                                                    ? <ChevronDown size={14} className="text-asvo-text-mid"/>
                                                    : <ChevronRight size={14} className="text-asvo-text-mid"/>
                                                }
                                            </button>
                                            <span className="font-bold text-asvo-text text-xs uppercase">
                                                {group.label}
                                            </span>
                                            <span className="text-[10px] text-asvo-text-dim ml-1">
                                ...
                                <th className={`
                                    ...
                                    bg-asvo-surface border-b
                                    sticky right-0 z-20
                                    shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]
                                `}>
                                    <div className="text-xs text-asvo-text-mid font-bold">Сохр.</div>
                                </th>
                            </tr>
                            ...
                            <tr>
                                <th className={`
                                    ${compactMode ? 'p-1' : 'p-2'}
                                    bg-asvo-surface border-b border-r
                                    sticky left-0 z-20
                                    shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]
                                `}></th>
                                ...
                                {groupedAbilities.map(group => (
                                    group.isExpanded ? (
                                        group.abilities.map(ab => (
                                            <th
                                                ...
                                                className={`
                                                    ...
                                                    text-center bg-asvo-surface border-b border-r align-top
                                                `}
                                            >
                                                <div className={`
                                                    ${compactMode ? 'text-[8px]' : 'text-[10px]'}
                                                    text-asvo-text-dim font-mono mb-0.5 truncate
                                                `} title={ab.code}>
                                                    .{ab.code.split('.')[1]}
                                                </div>
                                                <div className={`
                                                    ${compactMode ? 'text-[9px]' : 'text-xs'}
                                                    font-semibold text-asvo-text-mid leading-tight
                                        ...
                                        <th key={group.prefix + '-collapsed'} className="p-1 bg-asvo-surface border-b border-r">
                                            <div className="text-[10px] text-asvo-text-dim italic">свёрнуто</div>
                                        </th>
                                    )
                                ))}
                            ...
                            {roles.map(role => {
                                ...
                                return (
                                    ...
                                    >
                                        <td className={`
                                            ${compactMode ? 'p-2' : 'p-4'}
                                            border-r border-b bg-asvo-card
                                            sticky left-0 z-10
                                            group-hover:bg-indigo-50/30
                                            ${isChanged ? 'bg-amber-50/50 group-hover:bg-amber-100/50' : ''}
                                            shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]
                                        `}>
                                            <div className={`font-bold text-asvo-text ${compactMode ? 'text-xs' : 'text-sm'}`}>
                                                {role.description}
                                            </div>
                                                        ...
                                                        >
                                                            <div className="flex justify-center">
                                                                {isChecked
                                                                    ...
                                                                    : <Square size={compactMode ? 16 : 20} className="text-asvo-text-dim"/>
                                                                }
                                                            </div>
                                                        </td>
                                                    );
                                                        ...
                                                        return (
                                                            <div className={`
                                                                ...
                                                                ${checkedCount === total ? 'text-green-600' :
                                                                  checkedCount === 0 ? 'text-asvo-text-dim' : 'text-amber-600'}
                                            ...
                                            <button
                                                ...
                                                className={`
                                                    ...
                                                    ${isChanged && !isSuperAdmin
                                                        ...
                                                        : 'bg-asvo-surface-2 text-asvo-text-dim cursor-not-allowed'
                                                    }
                                                `}
            ...
            {/* Legend */}
            <div className="flex flex-wrap items-center gap-6 text-xs text-asvo-text-mid px-2">
                <div className="flex items-center gap-2">
                    <CheckSquare size={16} className="text-indigo-600"/>
                    <span>Право включено</span>
                </div>
                <div className="flex items-center gap-2">
                    <Square size={16} className="text-asvo-text-dim"/>

    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-fade-in">
        ...
        <div className="xl:col-span-8 space-y-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
                <button onClick={() => applyTemplate("THERMAL")} className="whitespace-nowrap px-4 py-2 bg-asvo-surface-2 border border-asvo-border rounded text-xs font-bold text-asvo-text-mid hover:bg-asvo-surface-3 transition flex gap-2"><Copy size={14}/> Пример: Тепловизоры</button>
                <button onClick={() => applyTemplate("SUB")} className="whitespace-nowrap px-4 py-2 bg-asvo-surface-2 border border-asvo-border rounded text-xs font-bold text-asvo-text-mid hover:bg-asvo-surface-3 transition flex gap-2"><Copy size={14}/> Пример: Субподряд</button>
                <button onClick={() => applyTemplate("CABLE")} className="whitespace-nowrap px-4 py-2 bg-asvo-surface-2 border border-asvo-border rounded text-xs font-bold text-asvo-text-mid hover:bg-asvo-surface-3 transition flex gap-2"><Copy size={14}/> Пример: Кабель</button>
            </div>

            <div className="bg-asvo-surface-2 p-6 rounded-2xl shadow-sm border border-asvo-border">
                ...
                <div className="mb-4">
                    <label className="block text-sm font-semibold text-asvo-text mb-2">Что принимаем?</label>
                    <div className="flex gap-2">
                        {(['PRODUCT', 'COMPONENT', 'OTHER'] as const).map(m => (
                            <button
                                key={m}
                                onClick={() => { setIntakeMode(m); setSelectedSkuId(""); setLabel(""); }}
                                className={`flex-1 py-2 rounded-lg border text-sm font-bold transition ${intakeMode === m ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-asvo-surface text-asvo-text-mid'}`}
                            >
                ...
                {(intakeMode === 'PRODUCT' || intakeMode === 'COMPONENT') && (
                    <div className="mb-4">
                        <label className="block text-sm font-semibold text-asvo-text mb-1">Выберите из справочника</label>
                        <select
                            className="w-full p-3 border border-asvo-border rounded-xl bg-asvo-surface text-asvo-text focus:ring-2 focus:ring-asvo-accent"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                     <div className="md:col-span-2">
                         <label className="block text-xs font-bold text-asvo-text-mid uppercase mb-1">Название</label>
                         <input value={label} onChange={e => setLabel(e.target.value)} className="w-full p-3 border border-asvo-border rounded-xl bg-asvo-surface text-asvo-text" placeholder="Например: Контроллер FC v2.4"/>
                     </div>
                     <div>
                         <label className="block text-xs font-bold text-asvo-text-mid uppercase mb-1">Партия</label>
                         <input value={batchName} onChange={e => setBatchName(e.target.value)} className="w-full p-2 border border-asvo-border rounded-lg text-sm bg-asvo-surface text-asvo-text" placeholder="№ партии" />
                     </div>
                     <div>
                         <label className="block text-xs font-bold text-asvo-text-mid uppercase mb-1">Проект</label>
                         <input value={projectName} onChange={e => setProjectName(e.target.value)} className="w-full p-2 border border-asvo-border rounded-lg text-sm bg-asvo-surface text-asvo-text" placeholder="Опционально" />
                     </div>
                     <div>
                         <label className="block text-xs font-medium text-asvo-text-mid mb-1 uppercase">Куда (Участок)</label>
                         <select value={intakeSectionId} onChange={e => setIntakeSectionId(Number(e.target.value))} className="w-full p-2 border border-asvo-border rounded-lg text-sm bg-asvo-surface text-asvo-text">
                            <option value="">-- Склад Приёмки --</option>
                            {sections.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                         </select>
                     </div>
                     <div>
                         <label className="block text-xs font-medium text-asvo-text-mid mb-1 uppercase">Статус</label>
                         <select value={intakeStatus} onChange={e => setIntakeStatus(e.target.value)} className="w-full p-2 border border-asvo-border rounded-lg text-sm bg-asvo-surface text-asvo-text">
                             <option value="ON_STOCK">На складе (Обычный)</option>
                             <option value="IN_WORK">Сразу в работу</option>
                         </select>
                     </div>
                     <div className="md:col-span-2">
                         <label className="block text-xs font-medium text-asvo-text-mid mb-1 uppercase">Примечание</label>
                         <input value={intakeNotes} onChange={e => setIntakeNotes(e.target.value)} className="w-full p-2 border border-asvo-border rounded-lg text-sm bg-asvo-surface text-asvo-text" placeholder="..." />
                     </div>
                </div>


                <div className="bg-asvo-accent-dim p-4 rounded-xl border border-asvo-border mb-6">
                    <div className="flex justify-between mb-2">
                        <h4 className="text-sm font-bold text-asvo-accent flex items-center gap-2"><Layers size={14}/> Расчет мест</h4>
                        <div className="flex text-[10px] font-bold bg-asvo-surface-2 rounded border border-asvo-border overflow-hidden">
                            <button onClick={() => setMeasureType("PCS")} className={`px-2 py-1 ${measureType === 'PCS' ? 'bg-asvo-accent-dim text-asvo-accent' : 'text-asvo-text-mid'}`}>ШТ</button>
                            <button onClick={() => setMeasureType("METERS")} className={`px-2 py-1 ${measureType === 'METERS' ? 'bg-asvo-accent-dim text-asvo-accent' : 'text-asvo-text-mid'}`}>МЕТРЫ</button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-2">
                         <div>
                             ...
                             <input type="number" min={1} value={shipmentTotal} onChange={e => setShipmentTotal(Number(e.target.value))} className="w-full p-2 border border-asvo-border rounded-lg font-bold bg-asvo-surface text-asvo-text" placeholder="0"/>
                         </div>
                         <div>
                             <label className="text-xs text-asvo-text-mid">В одной таре ({unit})</label>
                             <input type="number" min={1} value={capacityPerUnit} onChange={e => setCapacityPerUnit(Number(e.target.value))} className="w-full p-2 border border-asvo-border rounded-lg bg-asvo-surface text-asvo-text" placeholder="1"/>
                         </div>
                    </div>

                    <div className="text-sm text-asvo-accent font-medium flex justify-between items-center pt-2 border-t border-asvo-accent/30">
            ...
            <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        {createdBoxes.length > 0 && (
                            <button
                                ...
                                className="flex items-center gap-2 text-sm font-bold text-asvo-bg bg-asvo-accent px-3 py-2 rounded-lg hover:bg-asvo-accent/80 transition shadow-md"
                            >
                                <Printer size={18}/> PDF
                            </button>
                        )}
                ...
                {createdBoxes.length === 0 && <div className="text-center py-10 bg-asvo-surface-2 rounded-2xl border-2 border-dashed border-asvo-border text-asvo-text-dim">Здесь появятся этикетки</div>}

                <div className="grid grid-cols-2 gap-4 max-h-[600px] overflow-y-auto pr-2">
                    {createdBoxes.map(box => (
                        <div key={box.id} className="bg-asvo-surface-2 p-4 rounded-xl border border-asvo-border shadow-sm flex flex-col items-center text-center relative group hover:border-asvo-accent/40 transition">
                            <div className="absolute top-2 left-2 text-[10px] font-mono bg-asvo-surface-3 px-1.5 rounded text-asvo-text-mid font-bold">{box.shortCode}</div>
                            <div className="mb-2 bg-asvo-surface-2 p-1"><QRCode value={box.qrCode} size={80} /></div>
                            <div className="text-xs font-bold text-asvo-text w-full truncate">{box.label}</div>
                            <div className="text-[10px] text-asvo-text-mid mt-1">ID: {box.id}</div>
                            <div className="mt-2 bg-asvo-surface text-white text-xs font-bold px-2 py-0.5 rounded">{box.quantity} {box.unit}</div>
                        </div>
                    ))}
                </div>
            </div>

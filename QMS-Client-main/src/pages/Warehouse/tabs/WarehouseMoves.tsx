  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in">
        ...
        <div className="lg:col-span-5 space-y-4">
             ...
             <div className="bg-asvo-surface-2 p-4 rounded-xl shadow-sm border border-asvo-border">
                <label className="block text-sm font-bold text-asvo-text mb-2">Сканирование</label>
                <div className="flex gap-2">
                    <input
                        value={scanCode} onChange={e => setScanCode(e.target.value)}
                        onKeyDown={e => e.key === 'Enter' && handleScan()}
                        className="w-full p-3 border-2 border-asvo-border rounded-xl focus:border-asvo-accent text-lg outline-none bg-asvo-surface text-asvo-text"
             ...
             {currentBox ? (
                 <div className="bg-asvo-surface-2 rounded-xl shadow-lg border border-asvo-border overflow-hidden animate-fade-in">
                     <div className="bg-asvo-surface p-4 text-white">
                     ...
                     <div className="p-4 space-y-4">
                         ...
                         <div className="flex gap-2 bg-asvo-surface-3 p-1 rounded-lg">
                             <button onClick={() => setOpType("MOVE")} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition ${opType === 'MOVE' ? 'bg-asvo-surface-2 shadow text-asvo-accent' : 'text-asvo-text-mid'}`}>Перемещение</button>
                             <button onClick={() => setOpType("CONSUME")} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition ${opType === 'CONSUME' ? 'bg-asvo-surface-2 shadow text-asvo-amber' : 'text-asvo-text-mid'}`}>Расход / Списание</button>
                         </div>


                         {opType === 'CONSUME' ? (
                             <div className="bg-asvo-amber-dim p-3 rounded-lg border border-asvo-amber/30">
                                 <label className="text-xs font-bold text-asvo-amber uppercase">Сколько взять?</label>
                                 <div className="flex gap-2 mt-1">
                                    <input type="number" className="flex-1 p-2 text-xl font-bold border-asvo-amber/30 rounded outline-none bg-asvo-surface text-asvo-text" value={consumeAmount} onChange={e => setConsumeAmount(Number(e.target.value))} autoFocus />
                             ...
                             <div className="grid grid-cols-2 gap-3">
                                 <div>
                                     <label className="text-[10px] uppercase font-bold text-asvo-text-mid">Куда</label>
                                     <select className="w-full p-2 border border-asvo-border rounded text-sm bg-asvo-surface text-asvo-text outline-none" value={toSectionId} onChange={e => setToSectionId(e.target.value ? Number(e.target.value) : "")}>
                                         <option value="">(Текущее)</option>
                                         {sections.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                                     </select>
                                 </div>
                                 <div>
                                     <label className="text-[10px] uppercase font-bold text-asvo-text-mid">Статус</label>
                                     <select className="w-full p-2 border border-asvo-border rounded text-sm bg-asvo-surface text-asvo-text outline-none" value={statusAfter} onChange={e => setStatusAfter(e.target.value)}>
                         ...
                         <input className="w-full p-2 border border-asvo-border rounded text-sm outline-none focus:border-asvo-accent bg-asvo-surface text-asvo-text" placeholder="Комментарий..." value={comment} onChange={e => setComment(e.target.value)} />
        ...
        <div className="lg:col-span-7 h-[calc(100vh-140px)] flex flex-col">
            <div className="bg-asvo-surface-2 rounded-xl shadow-sm border border-asvo-border h-full flex flex-col overflow-hidden">
                <div className="p-4 border-b border-asvo-border bg-asvo-surface flex justify-between items-center">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-asvo-surface/30">
                    {cart.length === 0 && <div className="text-center py-20 text-asvo-text-dim">Список пуст</div>}
                    {cart.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-asvo-surface-2 border border-asvo-border rounded-lg shadow-sm hover:shadow-md transition group">

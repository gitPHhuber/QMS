...
export const WarehousePrintHistory: React.FC = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-asvo-surface-2 p-4 rounded-xl border border-asvo-border shadow-sm flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-3 flex-1 justify-end">
                    ...
                    <select
                        ...
                        className="p-2 border rounded-lg text-sm bg-asvo-surface font-medium"
                    >
            ...
            <div className="bg-asvo-surface-2 rounded-xl shadow-sm border border-asvo-border overflow-hidden">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-asvo-border text-sm">
                        {loading ? (
                            ...
                            data.map(item => (
                                <tr key={item.id} className="hover:bg-asvo-surface-3 transition group">
                                    <td className="px-6 py-4 text-asvo-text-mid whitespace-nowrap">
                                        {formatDateTime(item.createdAt)}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded-full bg-asvo-surface-3 flex items-center justify-center text-xs font-bold text-asvo-text-mid">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={clsx("text-xs font-bold px-2 py-0.5 rounded", item.template === "VIDEO_KIT" ? "bg-asvo-accent-dim text-asvo-accent" : "bg-asvo-surface-3 text-asvo-text")}>
                ...
                <div className="p-4 border-t border-asvo-border bg-asvo-surface flex justify-between items-center text-xs text-asvo-text-mid">
                    <span>Всего записей: {totalCount}</span>
                    <div className="flex gap-2">
                        <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-3 py-1 bg-asvo-surface-2 border rounded hover:bg-asvo-surface-3 disabled:opacity-50">Назад</button>
                        <span className="py-1 px-2">Стр. {page}</span>
                        <button disabled={data.length < 20} onClick={() => setPage(p => p + 1)} className="px-3 py-1 bg-asvo-surface-2 border rounded hover:bg-asvo-surface-3 disabled:opacity-50">Вперед</button>
                    </div>
                </div>
            </div>
            ...
            <Modal isOpen={!!selectedItem} onClose={() => setSelectedItem(null)}>
                {selectedItem && (
                    <div className="p-6 max-w-lg w-full">
                        ...
                        <div className="space-y-4 bg-asvo-surface p-4 rounded-xl border border-asvo-border text-sm">
                            ...
                            {selectedItem.params?.contract && (
                                <div className="border-t border-asvo-border pt-2 mt-2">
                                    ...
                                    <p className="bg-asvo-surface-2 p-2 rounded border border-asvo-border text-xs text-asvo-text italic">
                                        {selectedItem.params.contract}
                                    </p>
                                </div>
                            )}
                        ...
                        <button
                            ...
                            className="w-full mt-6 py-3 bg-asvo-surface-3 text-asvo-text font-bold rounded-xl hover:bg-asvo-surface-3 transition"
                        >
                            Закрыть
                        </button>
                    </div>

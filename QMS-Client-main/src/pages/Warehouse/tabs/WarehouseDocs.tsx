...
export const WarehouseDocs: React.FC = () => {
  ...
  const getTypeStyle = (type: string | null) => {
      ...
      if (t.includes("INCOME") || t.includes("ПРИХОД")) return "bg-asvo-green-dim text-asvo-green border-asvo-green/30";
      if (t.includes("OUT") || t.includes("РАСХОД") || t.includes("СПИСАНИЕ")) return "bg-asvo-amber-dim text-asvo-amber border-asvo-amber/30";
      return "bg-asvo-blue-dim text-asvo-blue border-asvo-blue/30";
  }
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-asvo-surface-2 p-4 rounded-xl border border-asvo-border shadow-sm flex flex-col md:flex-row justify-between gap-4">
            <div className="flex items-center gap-4 flex-1">
                ...
                <select
                    ...
                    className="p-2 border border-asvo-border rounded-lg bg-asvo-surface text-sm font-medium"
                >
        ...
        <div className="bg-asvo-surface-2 rounded-xl shadow-sm border border-asvo-border overflow-hidden">
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-asvo-surface border-b border-asvo-border text-xs font-semibold text-asvo-text-mid uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-asvo-border">
                    {loading ? (
                        ...
                        docs.map(doc => (
                            <tr key={doc.id} className="hover:bg-asvo-surface-3 transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-asvo-surface-3 rounded-lg text-asvo-text-mid">
                                            <FileText size={20} />
                                        </div>
                                        <div>

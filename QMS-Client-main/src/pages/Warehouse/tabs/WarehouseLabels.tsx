...
export const WarehouseLabels: React.FC = () => {
    ...
    return (
        <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-asvo-surface p-4 rounded-xl shadow-sm border border-asvo-border flex flex-col xl:flex-row justify-between gap-4">
                ...
                <div className="flex gap-2">
                    {selectedIds.length > 0 && (
                        <>
                            <button onClick={() => setIsEditOpen(true)} className="flex items-center gap-2 bg-asvo-amber-dim text-asvo-amber px-4 py-2 rounded-lg font-bold hover:bg-asvo-amber-dim transition">
                                <Edit size={18}/> Правка ({selectedIds.length})
                            </button>
                            <button onClick={handlePrint} className="flex items-center gap-2 bg-asvo-accent-dim text-asvo-accent px-4 py-2 rounded-lg font-bold hover:bg-asvo-accent-dim transition">
                                <Printer size={18}/> Печать
                            </button>
                        </>
                    )}
                    <button onClick={() => setIsCreateOpen(true)} className="flex items-center gap-2 bg-asvo-accent text-white px-4 py-2 rounded-lg font-bold hover:bg-asvo-accent/80 transition shadow-lg">
            ...
            <div className="bg-asvo-surface rounded-xl shadow-sm border border-asvo-border overflow-hidden">
                ...
                <div className="p-4 border-t border-asvo-border flex justify-between items-center bg-asvo-surface-2">
                    <span className="text-xs text-asvo-text-mid">Всего: {totalCount}</span>
                    <div className="flex gap-2">
                        <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-3 py-1 border rounded bg-asvo-surface disabled:opacity-50">Назад</button>
                        <button disabled={boxes.length < limit} onClick={() => setPage(p => p + 1)} className="px-3 py-1 border rounded bg-asvo-surface disabled:opacity-50">Вперед</button>
                    </div>
                </div>
            </div>
            ...
            <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)}>
                <div className="p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={handleCreate} className="w-full py-3 bg-asvo-accent text-white font-bold rounded-xl hover:bg-asvo-accent/80">Сгенерировать</button>
                    </div>
                </div>
            </Modal>

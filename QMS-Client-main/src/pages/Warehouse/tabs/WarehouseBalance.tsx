...
export const WarehouseBalance: React.FC = () => {
    ...
    const renderTable = (title: string, items: StockBalanceItem[], colorClass: string) => (
        <div className="mb-8 bg-asvo-surface-2 rounded-xl shadow-sm border border-asvo-border overflow-hidden">
            <div className={`px-6 py-3 font-bold text-asvo-text bg-asvo-surface border-b flex justify-between ${colorClass}`}>
                <span>{title}</span>
                <span className="text-xs bg-asvo-surface-2 px-2 py-0.5 rounded border">Позиций: {items.length}</span>
            </div>
            <table className="w-full text-left">
                <thead className="bg-asvo-surface/50 text-xs text-asvo-text-mid uppercase border-b">
                    <tr>
                ...
                <tbody className="divide-y divide-asvo-border text-sm">
                    {items.map((item, idx) => (
                        <tr key={idx} className="hover:bg-asvo-surface-3 transition">
    ...
    return (
        <div className="animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={loadData} className="p-2 bg-asvo-surface-3 rounded-full hover:bg-asvo-surface-3 transition"><RefreshCw size={18}/></button>
            </div>
            ...
            {loading ? (
                ...
                <>
                    ...
                    {balance.length === 0 && (
                        <div className="text-center py-20 bg-asvo-surface-2 rounded-xl border-2 border-dashed border-asvo-border">

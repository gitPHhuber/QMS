/**
 * WorkOrderListPage.tsx — Реестр производственных заданий
...
type WoStatus = "NEW" | "IN_PROGRESS" | "COMPLETED" | "CANCELLED";
...
const WorkOrderListPage: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    if (!createForm.title || createForm.title.length < 3) {
      setCreateError("Название: минимум 3 символа");
      return;
    }
    if (!createForm.dmrId) {
      setCreateError("Укажите ID DMR");
      return;
    }
    setCreating(true);
    try {
      await workOrderApi.create({
        ...
        dmrId: Number(createForm.dmrId),
        targetQty: Number(createForm.targetQty) || undefined,
        plannedStartDate: createForm.plannedStartDate || undefined,
        plannedEndDate: createForm.plannedEndDate || undefined,
      });
      setShowCreateModal(false);
      setCreateForm({ title: "", dmrId: "", targetQty: "", plannedStartDate: "", plannedEndDate: "" });
      fetchData();
    } catch (err: any) {
  ...
  const columns = [
    {
      ...
      label: "Название",
      render: (r: WoRow) => (
        <span className="font-mono text-asvo-accent font-semibold">{r.title}</span>
      ),
    },
    {
      key: "dmrNumber",
      label: "DMR",
      width: "120px",
      render: (r: WoRow) => <span className="text-asvo-text-mid font-mono">{r.dmrNumber}</span>,
    },
    {
      key: "targetQty",
      label: "Кол-во",
      ...
      render: (r: WoRow) => (
        <span className="text-asvo-text-mid">{r.targetQty}</span>
      ),
    },
    {
      key: "status",
      ...
      align: "center" as const,
      render: (r: WoRow) => {
        const s = STATUS_CFG[r.status] ?? STATUS_CFG.NEW;
    ...
    },
    {
      ...
      width: "150px",
      render: (r: WoRow) => (
        <span className="text-asvo-text-mid">{fmtDate(r.plannedStartDate)}</span>
      ),
    },
    {
      key: "responsibleName",
      label: "Ответственный",
      width: "160px",
      render: (r: WoRow) => <span className="text-asvo-text-mid">{r.responsibleName}</span>,
    },
  ];
    ...
    return (
      <div className="p-6 bg-asvo-bg min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-asvo-text-dim">Загрузка производственных заданий...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-5 bg-asvo-bg min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl" style={{ background: "rgba(74,144,232,0.12)" }}>
            <ClipboardCheck size={22} style={{ color: "#4A90E8" }} />
          </div>
          <div>
            ...
            <p className="text-xs text-asvo-text-dim">
              ISO 13485 &sect;7.5.1 &mdash; Управление производством
            </p>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <DataTable
          columns={columns}
          data={rows}
          onRowClick={(row: WoRow) => navigate(`/qms/work-orders/${row.id}`)}
        />
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-asvo-surface-2 border border-asvo-border rounded-xl w-full max-w-lg p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-asvo-text">Создать задание</h2>
              <button
                onClick={() => {
                  setShowCreateModal(false);
                  setCreateError(null);
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={createForm.dmrId}
                  onChange={(e) => setCreateForm((f) => ({ ...f, dmrId: e.target.value }))}
                  placeholder="Введите ID DMR"
                  className="w-full px-3 py-2 rounded-lg bg-asvo-surface border border-asvo-border text-[13px] text-asvo-text placeholder:text-asvo-text-dim focus:outline-none focus:border-asvo-accent transition"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreateForm((f) => ({ ...f, plannedStartDate: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreateForm((f) => ({ ...f, plannedEndDate: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg bg-asvo-surface border border-asvo-border text-[13px] text-asvo-text focus:outline-none focus:border-asvo-accent transition"
                />
              </div>
            </div>

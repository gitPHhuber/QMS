/**
 * DmrListPage.tsx — Реестр рецептов изделий (DMR)
 * Dark theme, ASVO-QMS design system
 * ISO 13485 §4.2.3 — Документы на устройства
 */
...
const DmrListPage: React.FC = () => {
  ...
  const [createForm, setCreateForm] = useState({ productId: "", title: "", description: "" });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await dmrApi.getAll(params);
      const list = (res.rows ?? res.data ?? []).map((r: any) => ({
        id: r.id,
        dmrNumber: r.dmrNumber ?? `DMR-${r.id}`,
        productTitle: r.product?.title ?? r.productTitle ?? "\u2014",
        version: r.version ?? 1,
        status: r.status ?? "DRAFT",
        approvedAt: r.approvedAt ?? null,
        authorName: r.author
          ? `${r.author.surname ?? ""} ${(r.author.name ?? "").charAt(0)}.`.trim()
          : r.authorName ?? "\u2014",
      }));
  ...
  const handleCreate = async () => {
    ...
    if (!createForm.productId) {
      setCreateError("Выберите изделие");
      return;
    }
    setCreating(true);
    try {
      ...
      setCreateForm({ productId: "", title: "", description: "" });
      fetchData();
    } catch (err: any) {
  ...
  const columns = [
    {
      ...
      width: "120px",
      render: (r: DmrRow) => (
        <span className="font-mono text-asvo-accent font-semibold">{r.dmrNumber}</span>
      ),
    },
    {
      key: "productTitle",
      label: "Продукт",
      render: (r: DmrRow) => <span className="text-asvo-text">{r.productTitle}</span>,
    },
    {
      key: "version",
      label: "Версия",
      ...
      render: (r: DmrRow) => (
        <span className="text-asvo-text-mid font-mono">v{r.version}</span>
      ),
    },
    {
      key: "status",
    ...
    },
    {
      ...
      width: "150px",
      render: (r: DmrRow) => (
        <span className="text-asvo-text-mid">{fmtDate(r.approvedAt)}</span>
      ),
    },
    {
      key: "authorName",
      label: "Автор",
      width: "160px",
      render: (r: DmrRow) => <span className="text-asvo-text-mid">{r.authorName}</span>,
    },
  ];
    ...
    return (
      <div className="p-6 bg-asvo-bg min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-asvo-text-dim">Загрузка реестра DMR...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-5 bg-asvo-bg min-h-screen">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-asvo-text">
              DMR &mdash; Реестр рецептов изделий
            </h1>
            <p className="text-xs text-asvo-text-dim">
              ISO 13485 &sect;4.2.3 &mdash; Device Master Record
            </p>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <DataTable
          columns={columns}
          data={rows}
          onRowClick={(row: DmrRow) => navigate(`/qms/dmr/${row.id}`)}
        />
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-asvo-surface-2 border border-asvo-border rounded-xl w-full max-w-lg p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-asvo-text">Создать DMR</h2>
              <button
                onClick={() => {
                  setShowCreateModal(false);
                  setCreateError(null);
